 ...
 *
 * Both successfully-submitted payments and pre-submission failures are persisted. Nothing displays
 * these rows any more — the hub's Recent list was removed — but they remain the app's only record
 * of a payment that failed before reaching the bank, and of a standing-order mandate at all, since
 * the AIS read side returns no id to correlate one on. Two lookups read them back: the rail a
 * payment was made on, and the timestamps behind the status screen's stage timeline.
 */
