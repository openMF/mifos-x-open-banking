...
import org.mifosx.openbanking.feature.paymentsstandingorder.generated.resources.feature_payments_standing_order_submitting_amount
import org.mifosx.openbanking.feature.paymentsstandingorder.generated.resources.feature_payments_standing_order_submitting_awaiting
 ...
 */
@Composable
internal fun StandingOrderSubmitting(
    stage: StandingOrderStage,
    amountLabel: String,
    creditorName: String,
    onAbandon: () -> Unit,
    ...
    val caption = when (stage) {
        StandingOrderStage.StagingConsent ->
            stringResource(Res.string.feature_payments_standing_order_submitting_staging)
        StandingOrderStage.AwaitingAuthorisation ->
            stringResource(Res.string.feature_payments_standing_order_submitting_awaiting)
    }
    ...
    Column(
        ...
        Text(
            text = stringResource(
                Res.string.feature_payments_standing_order_submitting_amount,
                amountLabel,
                creditorName,
            ),
        ...
        // stop it.
        if (stage == StandingOrderStage.AwaitingAuthorisation) {
            MifosTonalPillButton(
                label = stringResource(Res.string.feature_payments_standing_order_abandon_authorisation),
                onClick = onAbandon,
                testTag = StandingOrderTestTags.ABANDON_AUTHORISATION_BUTTON,
            )
        }
