...
object StandingOrderFixtures {
    ...
    fun loadingState(): StandingOrderState = StandingOrderState(uiState = StandingOrderUiState.Loading)
    ...
    fun formState(
        ...
        amountLabel: String = "",
        reference: String = "",
        problem: StandingOrderAmountProblem? = null,
        ...
        uiState = StandingOrderUiState.Content(
            ...
            firstPaymentDateLabel = firstPaymentDate?.let { EXECUTION_DATE_LABEL }.orEmpty(),
            datePickerRole = if (datePickerVisible) StandingOrderDateRole.First else null,
        ),
    )
    ...
    fun filledFormState(
        ...
        problem: StandingOrderAmountProblem? = null,
        firstPaymentDate: LocalDate? = EXECUTION_DATE,
    ): StandingOrderState = formState(
        firstPaymentDate = firstPaymentDate,
    ...
    fun reviewState(
        ...
        uiState = StandingOrderUiState.Content(
            ...
            firstPaymentDate = EXECUTION_DATE,
            firstPaymentDateLabel = EXECUTION_DATE_LABEL,
        ),
    )

    fun submittingState(
        stage: StandingOrderStage = StandingOrderStage.AwaitingAuthorisation,
    ): StandingOrderState = StandingOrderState(
        uiState = StandingOrderUiState.Submitting(
            stage = stage,
            amountLabel = "£850.00",
            creditorName = "Jameson Lettings",
            consentId = CONSENT_ID,
    ...
    fun errorState(
        kind: StandingOrderErrorKind = StandingOrderErrorKind.OutsideControlParameters,
