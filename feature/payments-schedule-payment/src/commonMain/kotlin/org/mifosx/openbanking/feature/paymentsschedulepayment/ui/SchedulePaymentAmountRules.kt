...
): SchedulePaymentAmountProblem? {
    val places = amount.decimalPlaces()
    val parsed = amountMinorUnits(amount)
    return when {
        amount.isBlank() -> null
        parsed == null -> SchedulePaymentAmountProblem.NotANumber
        places != null && places > MAX_DECIMAL_PLACES -> SchedulePaymentAmountProblem.TooManyDecimals
        parsed <= 0L -> SchedulePaymentAmountProblem.NotPositive
